...
export default function BonCommandePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!formData.fournisseur_nom.trim()) {
      ...
      const response = await bonCommandeService.create({
        ...formData,
        montant: formData.articles.reduce((sum, article) => sum + ((parseFloat(article.quantite) || 0) * (parseFloat(article.prix_unitaire) || 0)), 0),
        fournisseur: formData.fournisseur_nom.trim(),
        fournisseur_nom: formData.fournisseur_nom.trim(),
        fournisseur_adresse: formData.fournisseur_adresse.trim(),
        fournisseur_telephone: formData.fournisseur_telephone.trim(),
      ...
      setSuccessMessage(response?.message || 'Bon de commande créé avec succès !');
      setFormData({
        numero: `BC-${Date.now()}`,
        date_commande: new Date().toISOString().slice(0, 10),
        fournisseur_nom: '',
      ...
      const serverMessage = error?.message || error?.data?.message || error?.error || 'Erreur lors de la création du bon de commande';
      ...
      <div className="container-base">
        ...
        {successMessage && (
          <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => navigate('/bons-commande-list')} className="btn-primary">
                📋 Voir mes bons de commande
              </button>
              <button type="button" onClick={() => setSuccessMessage('')} className="btn-secondary">
                ✏️ Créer un autre bon de commande
              </button>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="card border-2 border-gray-200">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-sm text-gray-600">Tél: {COMPANY_INFO.phone}</p>
                <p className="text-sm text-gray-600">{COMPANY_INFO.email}</p>
              </div>
              <div className="text-right space-y-3">
                <p className="text-lg font-bold text-amber-600">BON DE COMMANDE N° {formData.numero}</p>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Date de commande</label>
                  <input type="date" name="date_commande" value={formData.date_commande} onChange={handleClientChange} className="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                ...
                <input type="text" name="fournisseur_adresse" value={formData.fournisseur_adresse} onChange={handleClientChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Téléphone <span className="text-red-500">*</span></label>
                <input type="tel" name="fournisseur_telephone" value={formData.fournisseur_telephone} onChange={handleClientChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Email</label>
                <input type="email" name="fournisseur_email" value={formData.fournisseur_email} onChange={handleClientChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500" />
              </div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <button type="button" onClick={addArticle} className="mt-4 flex items-center gap-2 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors">
            ...
            <div className="card">
              ...
              <div className="space-y-2">
                {TVA_OPTIONS.map((option) => (
                  <label key={option.value} className="flex items-center p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-amber-50">
          ...
          <div className="card bg-gray-50 border border-gray-300">
            <h3 className="text-lg font-bold text-gray-900 mb-4">Conditions du bon de commande</h3>
            <div className="space-y-3 text-sm text-gray-700">
              ...
              <p><span className="font-semibold">Durée du bon de commande :</span> Valable {QUOTE_VALIDITY.days} jours après l'obtention du bon de commande</p>
            </div>
          </div>
